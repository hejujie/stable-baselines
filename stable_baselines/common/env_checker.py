...
def check_env(env, warn=True):
    ...
    if warn and isinstance(action_space, spaces.Box):
        ...
        if action_space.low != action_space.high or np.abs(action_space.low) > 1 or np.abs(action_space.high) > 1:
    ...
    # because env inherits from gym.Env, we assume that
    obs = env.reset()
    ...
    # TODO: check metadata['render.modes']
